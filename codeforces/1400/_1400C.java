...
public class _1400C{
    ...
    static int i() throws IOException { return Integer.parseInt(bf.readLine().trim());}
    static int[] ni() throws IOException { 
        String[] d = bf.readLine().trim().split(" ");
        int[] s = new int[d.length];
        for(int i = 0,h=d.length;i<h;i++)
            s[i] = Integer.parseInt(d[i]);
        return s;
    }
    ...
    static int lcs(String a,String b){
        ...
        for(int i = 1,h=a.length() + 1;i<h;i++){
            for(int j = 1,v=b.length() + 1;j<v;j++){
                dp[i][j] = a.charAt(i) ==  b.charAt(j) ? dp[i-1][j-1] + 1 : Math.max(dp[i][j-1],dp[i-1][j]);
            }
        }
