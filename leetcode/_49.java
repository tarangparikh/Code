...
public class _49 {
	...
	List<List<String>> solve(String[] strs) {
		TreeMap<int[], List<String>> map = new TreeMap<>(
			(a, b) -> {
				for (
		...
		for (Map.Entry<int[], List<String>> entry : map.entrySet()) list.add(
			entry.getValue()
		);
		return list;
	}
	...
	List<List<String>> solve1(String[] strs) {
		...
		for (String str : strs) {
			...
			for (char e : str.toCharArray()) f[e - 'a']++;
			long key = hash(f);
			if (!map.containsKey(key)) map.put(key, new ArrayList<>());
			map.get(key).add(str);
		}
		...
		List<List<String>> list = new ArrayList<>();
		for (Map.Entry<Long, List<String>> entry : map.entrySet()) list.add(
			entry.getValue()
		);
